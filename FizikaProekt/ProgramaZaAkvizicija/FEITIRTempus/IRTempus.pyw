...
class MyForm(QtGui.QDialog):
	def __init__(self, parent=None):
		...
		pixmap = QPixmap(os.getcwd() + "/ukim.jpg")
		self.ui.l_ufscar.setPixmap(pixmap)
		self.ui.l_ufscar.show()
		pixmap = QPixmap(os.getcwd() + "/feit.jpg")
	...
	def findport(self):	#finds automatically the serial port connected to arduino using the msg_co and msg_ok messages
		...
		self.ui.l_commstatus.setText("Komunikacija so Arduino plocata...")
		...
		if echo[0] == msg_ok :
			...
			self.ui.l_commstatus.setText("Uspesna komunikacija so Arduino plocata na seriska porta: " + port + ".")
			sport.flushInput()
			sport.flushOutput()
		else:
			self.ui.l_commstatus.setText("Neuspesna komunikacija so Arduno plocata. Ne e pronajdena seriska porta..")
	...
	def closeconnection(self):	#close connection to the serial port if it's already opened
		...
		if echo[0] == msg_ok :
			...
			sport.close()
			echo = ["",""]
			self.ui.l_commstatus.setText("Komunikacijata so Arduino plocata e prekinata.")
	...
	def setport(self, port):	#set a serial port chosen by the user
		...
		try:			
			...
			if res == msg_ok :
				echo = [msg_ok, p]
				self.ui.l_commstatus.setText("Uspesna komunikacija so Arduino plocata na seriska porta: " + p + ".")
				sport.flushInput()
				sport.flushOutput()
			else:
				sport.close()
				self.ui.l_commstatus.setText("Arduino plocata ne e pronajdena na seriska porta:" + p + ".")

		except (OSError, serial.SerialException):
			self.ui.l_commstatus.setText(p + " seriskata porta nemoze da bide otvorena")
	...
	def startacq(self):
		...
		if runningflag == False:
			...
			if echo[0] == msg_ok :	#Verify if the connection to arduino was already stablished
				...
				self.ui.l_mostrador.setText("raboti...")
				self.ui.pB_startacq.setText("Se snima")
				...
				self.ui.pB_startacq.setText("Start")
				self.ui.pB_startacq.setStyleSheet("color: white; background-color: green;")
				self.ui.l_mostrador.setText("Kraj.")

				self.ui.l_filename.setText("*Datoteka: " + fdisp)


			else:
				win = Alertparam("Arduino plocata ne e povrzana!")
				win.show()
	...
	def SaveFile(self):
		...
		if len(data[0]) > 2:
			...
			filename = QtGui.QFileDialog.getSaveFileName(self, 'Zacuvaj branovi oblici', '.')
			...
			fname.write("Vreme(s)\tCanal_1\tCanal_2\tCanal_3\tCanal_4\tCanal_5\tCanal_6\n")
			...
			win = Alertparam("Nema podatoci za zacuvuvanje!")
			win.show()
	...
	def LoadFile(self):
		...
		filename = QtGui.QFileDialog.getOpenFileName(self, 'Procitaj branovi oblici', '~')
		...
		try:
			...
			win = Alertparam("Nepoznat format na datoteka!")
			win.show()
	...
	def SaveFileTime(self):
		...
		if dtime != []:
			...
			filename = QtGui.QFileDialog.getSaveFileName(self, 'Zacuvaj tabela so vremenski intervali', '.')
			fname = open(filename, 'w')				#Signal data
